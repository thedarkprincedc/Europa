...
CPanelEx::CPanelEx(IDirect3DDevice9 *pDevice, char *pszFilename, 
...
float CPanelEx::getWidth(){		
	return m_fWidth;	
}
float CPanelEx::getHeight(){	
	return m_fHeight;	
}
float CPanelEx::getX(){	
	return m_fx; 
...
BOOL CPanelEx::CreateVertexBuffer(IDirect3DDevice9 *pDevice){
	if(FAILED(pDevice->CreateVertexBuffer(4*sizeof(PANEL_CUSTOMVERTEX),
