...
#include <d3d9.h>
#include <d3dx9.h>
#define D3DFVF_PARTICLES (D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX1)
typedef struct _tagBillboardParticleVertex{
...
} BILLBOARDPARTICLEVERTEX;

class CEmitter{
public:
	CEmitter(int numParticles, D3DXVECTOR3 Origin);
	virtual ~CEmitter();
	virtual HRESULT	Render(IDirect3DDevice9 *pDevice) = 0;
	void SetOrigin(float x, float y, float z){
		m_vOrigin = D3DXVECTOR3(x,y,z);
	}
	void SetOrigin(D3DXVECTOR3 vect){
		m_vOrigin = vect;
	}
	BILLBOARDPARTICLEVERTEX	   *m_akParticles;
...
class CParticleExplosion : public CEmitter {
public:
	CParticleExplosion(IDirect3DDevice9 *pDevice, int numParticles = 1000,
		D3DXVECTOR3 Origin = D3DXVECTOR3(0,0,0), char *pszTexture = "star.bmp");
	virtual ~CParticleExplosion();
	HRESULT Render(IDirect3DDevice9 *pDevice);
};
class CParticleHole : public CEmitter {
public:
	CParticleHole(IDirect3DDevice9 *pDevice, int numParticles = 1000,
		D3DXVECTOR3 Origin = D3DXVECTOR3(0,0,0), char *pszTexture = "star.bmp")
	...
	HRESULT Render(IDirect3DDevice9 *pDevice){
		...
		//{
			for(int iCount = 0; iCount < m_iNumParticles * 4; iCount += 4){
				if(m_akParticles[iCount].m_fAge == 0){
				// I'm sure you can work this code out. Just remember it's a quad!
				m_akParticles[iCount].m_fX = -0.5f;
				m_akParticles[iCount].m_fY = -0.5f;
				m_akParticles[iCount].m_fZ = 0.0f;
