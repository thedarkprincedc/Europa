...
CAnimSprite::CAnimSprite(IDirect3DDevice9 *pDevice, char *pImgfilename, D3DCOLOR color)
	:	CSpriteDx(pDevice, pImgfilename, color), 
	m_OffsetX(0), m_OffsetY(0), nextframe(0){
}

CAnimSprite::CAnimSprite(IDirect3DDevice9 *pDevice, char *pImgfilename, int numFrames, int frameWidth, int frameHeight, 
			BOOL playOnce, long timeIntervalMillisec, ANIMDIRECTION directionAnim, D3DCOLOR color)
			:	CSpriteDx(pDevice, pImgfilename, color), m_OffsetX(0), m_OffsetY(0), nextframe(0){
	SetupAnimation(numFrames, frameWidth, frameHeight, playOnce, timeIntervalMillisec, directionAnim);
...
void CAnimSprite::SetupAnimation(int numFrames, int frameWidth, int frameHeight, 
	...
	m_Timer = new CObjectTimer(timeIntervalMillisec);
	m_AnimDirection = directionAnim;
...
//**************************************************
//******************************************
/*CSprite::CSprite(IDirect3DDevice9 *pDevice, char *pImgFilename, D3DCOLOR KeyColor, float x, float y)
	:	CPanelEx(pDevice, pImgFilename, KeyColor, x, y){	}
CSprite::CSprite(IDirect3DDevice9 *pDevice, IDirect3DTexture9 *pTexture, float x, float y)
	:	CPanelEx(pDevice, pTexture){	}
//CSprite::CSprite(const CSprite &a) : CPanelEx(a){ }
CSprite::~CSprite(){	}
