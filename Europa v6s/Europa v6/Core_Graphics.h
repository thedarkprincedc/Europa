...
void SetUp2DCamera(IDirect3DDevice9 *pD3DDevice, float m_ScreenWidth, float m_ScreenHieght);
static INT64 _FREQUENCY = 0;
static INT64 _NEWCOUNT = 0;
static INT64 _LASTCOUNT = 0;
static void INIT_FRAMERATE(){
	QueryPerformanceFrequency((LARGE_INTEGER*)&_FREQUENCY);
	if(_FREQUENCY == 0)	OutputDebugString(L"This System Doesnt Support High Resolution Timers");
	QueryPerformanceCounter((LARGE_INTEGER*)&_NEWCOUNT);
	if(_NEWCOUNT == 0)	OutputDebugString(L"This System Doesnt Support High Resolution Timers");	
}
static BOOL REGULATE_FRAMERATE(){
	QueryPerformanceCounter((LARGE_INTEGER*)&_NEWCOUNT);
	if((_NEWCOUNT - _LASTCOUNT) > 60*1000){	// RENDER a frame if 60 milliseconds have passed
...
//***************************************************
class GroupNode{ };// Effects Sprites/etc
class SceneGraph{
public:
	void AttachGroupNode(GroupNode *pGroupNode){
		if(pGroupNode != NULL)
			m_vpGroupNodes.push_back(pGroupNode);
	}
protected:
	vector<GroupNode*> m_vpGroupNodes;
};
//*************************************************
class CGraphics;
class CMesh;
